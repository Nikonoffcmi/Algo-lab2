...
int main(){ 
    ...
    // ofstream output_file("./write_example_uniform.txt");

    // ostream_iterator<int> output_iterator(output_file, "\n");
    // copy(begin(insertions), end(insertions), output_iterator);
    
    int element;
    std::ifstream fin("write_example_uniform.txt");
    
    int numLinesToRead = 43; // Количество строк для чтения
    std::string line;
    for (int i = 0; i < numLinesToRead && std::getline(fin, line); ++i) {
        int num = std::atoi(line.c_str()); // Преобразуем строку в целое число
        myHash.insert(num); 
        myHash2.insert(num);
    }
    ...
    // ofstream output_file("./search_example.txt");

    // ostream_iterator<int> output_iterator(output_file, "\n");
    // copy(begin(queries), end(queries), output_iterator);

    std::ifstream fin2("search_example.txt");
    while (fin2 >> element)
    {
